...
let BattleMovedex = {
	...
	"gravity": {
		...
		basePower: 0,
		category: "Status",
		desc: "For 5 turns, the evasiveness of all active Pokemon is multiplied by 0.6. At the time of use, Bounce, Fly, Magnet Rise, Sky Drop, and Telekinesis end immediately for all active Pokemon. During the effect, Bounce, Fly, Flying Press, High Jump Kick, Jump Kick, Magnet Rise, Meteor Kick, Sky Drop, Splash, and Telekinesis are prevented from being used by all active Pokemon. Ground-type attacks, Spikes, Toxic Spikes, Sticky Web, and the Ability Arena Trap can affect Flying types or Pokemon with the Ability Levitate. Fails if this move is already in effect. Space-type Pokémon can use Moves that do not work under Gravity's effect.",
		shortDesc: "For 5 turns, negates all Ground immunities. Reduces the evasiveness of all active pokemon.",
		id: "gravity",
		name: "Gravity",
		pp: 5,
		priority: 0,
	...
	"hyperbeam": {
		...
		basePower: 150,
		category: "Special",
		desc: "If this move is successful, the user must recharge on the following turn and cannot make a move. If this move successfully knocks out the target, there is no recharge on the following turn.",
		shortDesc: "User cannot move next turn, unless this move knocks out the target.",
		id: "hyperbeam",
		name: "Hyper Beam",
		pp: 5,
		priority: 0,
	...
	"lightthatburnsthesky": {
		...
		basePower: 200,
		category: "Special",
		desc: "Removes weathers, terrains and deactivates the following Abilities if the target has them: Desolate Land, Primordial Sea, Delta Stream. These Abilities remain deactivated until the target switches out.",
		shortDesc: "Removes weathers and terrains.",
	...
	"rapidspin": {
		...
		self: {
			onHit: function (pokemon) {
				...
				let sideConditions = ['spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'healingstones'];
	...
	"signalbeam": {
		...
		basePower: 75,
		category: "Special",
		desc: "Confuses the target if they have a positive stat boost.",
		shortDesc: "Confuses the target if they have a positive stat boost.",
		id: "signalbeam",
		isViable: true,
		name: "Signal Beam",
		pp: 15,
	...
	"solarbeam": {
		...
		onTry: function (attacker, defender, move) {
			...
			this.add('-prepare', attacker, move.name, defender);
			if (this.field.isWeather(['sunnyday', 'desolateland']) || !this.runEvent('ChargeMove', attacker, defender, move)) {
				this.add('-anim', attacker, move.name, defender);
				return;
			}
			attacker.addVolatile('twoturnmove', defender);
	...
	"spacialrend": {
		...
		basePower: 100,
		category: "Special",
		desc: "Removes field effects such as normal weathers, rooms and terrains after a successful hit. (Can remove Sun, Rain, Sandstorm, Hail, Trick Room, Magic Room, Wonder Room, any of type Terrains, and Gravity.)",
		shortDesc: "Removes field effects after a successful hit.",
		id: "spacialrend",
		isViable: true,
		name: "Spacial Rend",
		pp: 5,
	...
	"stealthrock": {
		...
		basePower: 0,
		category: "Status",
		desc: "Sets up a hazard on the foe's side of the field, damaging each foe that switches in. Can be used twice before failing. Foes lose 1/64, 1/32, 1/16, 1/8, or 1/4 of their maximum HP, rounded down, based on their weakness to the Rock type; 0.25x, 0.5x, neutral, 2x, or 4x, respectively. Does double damage if two layers are set up. Can be removed from the foe's side if any foe uses Rapid Spin or Defog, is hit by Defog, or if any opponent rock-typed pokemon switches in.",
	...
	"healingterrain": {
		...
		basePower: 0,
		category: "Status",
		desc: "For 5 turns (or 8 turn with Terrain Extender), any Pokémon that switches out will have their HP healed by 25% of their max HP (no effect on Pokémon with Regenerator, which heals by 33% as usual), and Dark Status Moves with a Priority of 0 or greater will have -1 Priority.",
		shortDesc: "5 turns. +1/4 max HP restored on switch-out. Dark Priority moves are now -1 Priority.",
		id: "healingterrain",
	...
	"medicinethrow": {
		...
		basePower: 80,
		category: "Special",
		desc: "If the target is an ally, medicine throw will heal instead of dealing damage. Heal amount is based on Heart's type effectiveness to the target.",
		shortDesc: "If the target is an ally, heals by Heart's type effectiveness to the target",
		id: "medicinethrow",
		name: "Medicine Throw",
		pp: 10,
		priority: 0,
	...
	"healingstones": {
		...
		basePower: 0,
		category: "Status",
		desc: "Sets healing stones on the user's side, healing grounded Pokemon that switch in for 1/8th of their max HP. Healing stones are considered a healing move. Can be removed by Rapid Spin and Defog.",
	...
	"sedation": {
		...
		basePower: 20,
		category: "Physical",
		desc: "Causes the target to fall asleep at the end of the next turn. Allied targets affected by this move will have 1/2 of their maximum HP restored. Fails when used if the target cannot fall asleep or if it already has a major status condition. At the end of the next turn, if the target is still on the field, does not have a major status condition, and can fall asleep, it falls asleep. This effect cannot be prevented by Safeguard, Substitute, or by falling asleep and waking up during the effect.",
		shortDesc: "Puts the target to sleep after 1 turn. Also heals allies by 1/2 of their max HP.",
		id: "sedation",
		name: "Sedation",
		pp: 20,
		priority: 0,
	...
	"fakemedics": {
		...
		basePower: 0,
		category: "Status",
		desc: "Inflicts necrosis. Necrosis deals 1/16 HP damage per turn. Pokemon affected by Necrosis can't heal.",
	...
	"vitalwave": {
		...
		basePower: 60,
		category: "Physical",
		desc: "Heals allied targets by the amount of damage they would have received, instead of damaging them.",
	...
	"carepackage": {
		...
		basePower: 0,
		category: "Physical",
		desc: "-1 Priority. Switches the user out. If the user is holding a Berry or Herb-based consumable item, the incoming allied Pokémon gains its effects. Using this move does not expend the user's berry.",
